...
class _NovaPreVendaScreenState extends State<NovaPreVendaScreen> {
  ...
  Future<void> _adicionarProdutoPorCodigo(String codigo) async {
    ...
    final produto = lista.first;
    // Mostra o código lido + produto encontrado sempre — permite perceber na
    // hora se o leitor pegou o código errado (ex: leu "88029" quando a
    // etiqueta era "88629"), antes de o item entrar na venda por engano.
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Lido: $codigo → ${produto.nome ?? "—"} (Cód: ${produto.pkChave})'),
        duration: const Duration(seconds: 3),
        backgroundColor: AppTheme.primary,
      ),
    );
    _adicionarOuIncrementar(produto);
  }
...
class _BuscaProdutoDialogState extends State<_BuscaProdutoDialog> {
  ...
  void _buscar(String termo) {
    _debounce?.cancel();
    final termoTrim = termo.trim();
    final codigoExato = int.tryParse(termoTrim);
    // Um código (só dígitos) já pode buscar com 1 caractere — é busca exata,
    // não varredura por substring. Texto continua exigindo 2+ caracteres.
    if (termoTrim.isEmpty || (codigoExato == null && termoTrim.length < 2)) {
      setState(() => _resultados = []);
      return;
    }
    _debounce = Timer(const Duration(milliseconds: 400), () async {
      setState(() => _buscando = true);
      try {
        List<ProdutoResumo> r;
        if (codigoExato != null) {
          // Se digitou só números, tenta achar EXATAMENTE o produto com esse
          // código — evita que "1" traga qualquer produto cujo nome/referência
          // contenha "1". Só cai pra busca por texto se não achar nada com
          // esse código (ex: o número também aparece em alguma referência).
          r = await widget.service.listarProdutos(pkChaveExato: codigoExato, limit: 1);
          if (r.isEmpty) {
            r = await widget.service.listarProdutos(busca: termoTrim, limit: 30);
          }
        } else {
          r = await widget.service.listarProdutos(busca: termoTrim, limit: 30);
        }
        ...
        child: Column(
          ...
          children: [
            ...
            TextField(
              ...
              autofocus: true,
              decoration: InputDecoration(
                hintText: 'Código do produto, nome ou referência...',
                prefixIcon: const Icon(Icons.search),
                suffixIcon: _buscando
                    ? const Padding(padding: EdgeInsets.all(12), child: SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)))
                    : null,
